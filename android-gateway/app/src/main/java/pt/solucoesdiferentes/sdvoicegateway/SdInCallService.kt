...
class SdInCallService : InCallService() {
    ...
    override fun onCallAdded(call: Call) {
        super.onCallAdded(call); currentCall = call; probedThisCall = false
        saveDiagnostic("CALL", "Call detected state=${call.state}"); call.registerCallback(callback); reportState(call.state)
    }
    override fun onCallRemoved(call: Call) {
        call.unregisterCallback(callback); report("ENDED"); saveDiagnostic("CALL", "Call ended")
        if (currentCall === call) currentCall = null; probedThisCall = false; super.onCallRemoved(call)
    }
    ...
    private fun reportState(state: Int) {
        ...
        when (state) {
            ...
            Call.STATE_ACTIVE -> { report("ACTIVE"); if (!probedThisCall) { probedThisCall = true; probeCallAudio() } }
    ...
    private fun probeCallAudio() {
        Thread {
            // Capture all local snapshots first while the cellular call is definitely ACTIVE.
            val devicePayload = JSONObject().put("manufacturer", android.os.Build.MANUFACTURER).put("model", android.os.Build.MODEL)
                .put("device", android.os.Build.DEVICE).put("hardware", android.os.Build.HARDWARE).put("board", android.os.Build.BOARD)
                .put("sdk", android.os.Build.VERSION.SDK_INT).put("mode", "STOCK_ONLY")
            ...
            val pcmPayload = try { TelephonyPcmDiagnostics.collect(this) } catch (t: Throwable) {
                JSONObject().put("mode", "STOCK_ONLY").put("error", t.message ?: t.javaClass.simpleName)
            }
            ...
            // Only after snapshots are captured do we perform network I/O.
    ...
    private fun stateName(state: Int) = when (state) {
        Call.STATE_NEW -> "NEW"; Call.STATE_DIALING -> "DIALING"; Call.STATE_RINGING -> "RINGING"; Call.STATE_HOLDING -> "HOLDING"
        Call.STATE_ACTIVE -> "ACTIVE"; Call.STATE_DISCONNECTED -> "DISCONNECTED"; Call.STATE_CONNECTING -> "CONNECTING"
        Call.STATE_DISCONNECTING -> "DISCONNECTING"; Call.STATE_SELECT_PHONE_ACCOUNT -> "SELECT_PHONE_ACCOUNT"; else -> "STATE_$state"
    }
    companion object {
        @Volatile var pendingCallId: String? = null; @Volatile var pendingCommandId: String? = null; @Volatile private var currentCall: Call? = null
        fun hangupCurrentCall(context: Context, callId: String?) { currentCall?.disconnect(); try { GatewayApi.event(context, "ENDED", callId ?: pendingCallId, pendingCommandId) } catch (_: Throwable) {} }
    }
}

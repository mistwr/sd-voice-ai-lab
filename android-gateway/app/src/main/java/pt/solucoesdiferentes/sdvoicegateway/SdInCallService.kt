...
class SdInCallService : InCallService() {
    ...
    override fun onCallAdded(call: Call) { super.onCallAdded(call); currentCall=call; probedThisCall=false; recordingStarted=false; saveDiagnostic("CALL","Call detected state=${call.state}"); call.registerCallback(callback); reportState(call.state) }
    override fun onCallRemoved(call: Call) { if(recordingStarted){sendCapability("CALL_RECORDING_STOPPED",CallSessionRecorder.stop());recordingStarted=false}; call.unregisterCallback(callback); report("ENDED"); saveDiagnostic("CALL","Call ended"); if(currentCall===call) currentCall=null; probedThisCall=false; super.onCallRemoved(call) }
    private val callback=object:Call.Callback(){ override fun onStateChanged(call:Call,state:Int){reportState(state)} }
    private fun reportState(state:Int){ saveDiagnostic("STATE",stateName(state)); when(state){ Call.STATE_DIALING,Call.STATE_CONNECTING->report("DIALING"); Call.STATE_RINGING->report("RINGING"); Call.STATE_ACTIVE->{report("ACTIVE");startAiSessionIfEnabled();if(!probedThisCall){probedThisCall=true;probeCallAudio()}}; Call.STATE_DISCONNECTED->{if(recordingStarted){sendCapability("CALL_RECORDING_STOPPED",CallSessionRecorder.stop());recordingStarted=false};report("ENDED")} } }
    private fun startAiSessionIfEnabled(){val p=getSharedPreferences("gateway",Context.MODE_PRIVATE);val record=p.getBoolean("record_calls",true);val transcribe=p.getBoolean("transcribe_calls",true);sendCapability("AI_SESSION_STARTED",JSONObject().put("recording_requested",record).put("transcription_requested",transcribe).put("mode","STOCK_ONLY"));if(record&&!recordingStarted){val result=CallSessionRecorder.start(this);recordingStarted=result.optBoolean("recording",false);sendCapability("CALL_RECORDING_STARTED",result)};if(transcribe)sendCapability("TRANSCRIPTION_REQUESTED",JSONObject().put("enabled",true).put("status","WAITING_FOR_BIDIRECTIONAL_PCM").put("note","Do not fabricate transcript when call PCM is unavailable"))}
